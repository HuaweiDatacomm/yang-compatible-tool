...
def validate_path_expr(errors, path):
    ...
    def parse_keypath(s):
        ...
        def parse_identifier(s):
            #Increase the prefix in the yin file processing, and handle the false positives in the import invalid
            prefix_list = []
            try:
                s_list = s.split("/")
                for sub_s in s_list:
                    if sub_s:
                        m_sub = syntax.re_keyword_start.match(sub_s)
                        if m_sub and m_sub.group(2):
                            prefix_list.append(m_sub.group(2))
