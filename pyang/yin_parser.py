    ...
    def create_statement(self, e, parent):
        if e.ns == yin_namespace:
            ...
            try:
                ...
                key_flag = False
                if 'unfound' in self.prefixmap:
                    for (mod_name, prefix) in self.prefixmap["unfound"]:
                        if e.ns.endswith(mod_name):
                            key_flag = True
                            break
                if not key_flag:
                    error.err_add(self.ctx.errors, e.pos,
                    ...
                    if mod is not None:
                        ...
                        try:
                            ...
                            if p is not None:
                                prefix = p.find_attribute('value')
                                if prefix is not None:
                                    if 'unfound' not in self.prefixmap:
                                        self.prefixmap['unfound'] = []
                                    self.prefixmap['unfound'].append((modname, prefix))
                        except:
                            pass
