        ...
        if res == 'continue':
            ...
            if phase in _v_i_children:
                ...
                for s in stmt.substmts:
                    if s.keyword in ('when', 'must', 'deviation'):
                        check_prefix_in_deviate_must(s)
                ...
                for s in stmt.substmts:
                    ...
                    if s.keyword in ('when', 'must', 'deviation'):
                        check_prefix_in_deviate_must(s)
                    # deal with prefix with ext...
                    elif isinstance(s.keyword, tuple):
                        deal_with_prefix_with_ext(s)
...
def pattern_prefix_with_ext(node_list):
    prefix_list = []
    try:
        prefix_partten = re.compile(r"[/\( ']*([_A-Za-z][._\-A-Za-z0-9]*):")
        for sub_node in node_list:
            if ':' not in sub_node:
                continue
            sub_prefix = ''.join([sub_node.split(':')[0], ':'])
            prefix_match = prefix_partten.search(sub_prefix)
            if prefix_match:
                prefix_list.append(prefix_match.group(1))
    except:
        return []
    return prefix_list


# Process the xpath involved in the private grammar to match the prefix used
def deal_with_prefix_with_ext(s):
    try:
        if s.arg.startswith("/"):
            arg = s.arg
        else:
            arg = "/" + s.arg
        node_list = arg.split('/')[1:]
        prefix_list = pattern_prefix_with_ext(node_list)
        # remove duplicates
        prefix_list = list(set(prefix_list))
        remove_unsued_import_with_ext(prefix_list, s.i_orig_module)
    except:
        pass


# Remove the xpath of the private grammar and the prefix involved in the statement from i_unused_prefixes
def remove_unsued_import_with_ext(prefix_list, module):
    for prefix in prefix_list:
        if prefix in module.i_unused_prefixes:
            del module.i_unused_prefixes[prefix]


def check_prefix_in_deviate_must(c):
    try:
        if c.arg.startswith("/"):
            arg = c.arg
        else:
            arg = "/" + c.arg # to make node_id_part below work
        # parse the path into a list of two-tuples of (prefix,identifier)
        pattern_new = re.compile(r"[/\( '](([_A-Za-z][._\-A-Za-z0-9]*):)?([_A-Za-z][._\-A-Za-z0-9]*)")
        path = [(m[1], m[2]) for m in pattern_new.findall(arg)]
        # find the module of the first node in the path
        (prefix, identifier) = path[0]
        prefix_list = [s_path[0] for s_path in path]
        prefix_list = list(set(prefix_list))
        if len(prefix_list) > 1:
            for sub_prefix in prefix_list:
                if sub_prefix and sub_prefix != c.i_orig_module.i_prefix:
                    prefix = sub_prefix
                    break
        util.prefix_to_modulename_and_revision(c.i_orig_module, prefix, c.pos, [], prefix_list)
    except:
        pass
...
def find_target_node(ctx, stmt, is_augment=False):
    ...
    prefix_list = [s_path[0] for s_path in path]
    prefix_list = list(set(prefix_list))
    module = prefix_to_module(stmt.i_module, prefix, stmt.pos, ctx.errors, prefix_list)
    if module is None:
